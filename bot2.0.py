...
def get_cached_today_classes(session):
    ...
    if isinstance(classes, list):
        ...
        print(f"[{datetime.now(IST).strftime('%H:%M')}] Timetable cached for {today} ({len(classes)} classes)")
    return classes
...
async def send_timetable():
    ...
    await user.send(msg)
    print(f"[{datetime.now(IST).strftime('%H:%M')}] Sent timetable + attendance to {user}")
